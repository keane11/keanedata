---
title: Harness Engineering 工作流完整指南
description: 基于 Hashimoto/OpenAI/Anthropic 实践整理的 AI Agent 驾驭工程六层框架，涵盖上下文工程、结构化执行、持久化记忆、架构约束、反馈循环与债务管理。
tags: [AI, Harness Engineering, Agent, workflow]
date: 2026-05-15
order: 6
---

# Harness Engineering 工作流完整指南

> **版本:** v1.1  
> **理论基础:** Mitchell Hashimoto (2026.2) · OpenAI 百万行代码实践 · Anthropic 长时间运行 Agent 研究  
...
| 数据来源 | 发现 |
|:--|:--|
| OpenAI 内部实践 | 百万行代码仓库中，Agent 失败案例有 **63%** 源于上下文不完整，而非模型能力不足 |
| Anthropic 长时间运行研究 | Agent 任务失败的主因是**基础设施问题**（进度丢失、工具调用失败），不是推理错误 |
| Hashimoto / Ghostty 项目 | 大部分 Agent debug 时间花在"**恢复状态**"而非"实际调试"——上次停在哪、改了什么、为什么 |
| Stripe 工程实践 | 没有结构化记忆的 Agent 在多会话任务中**重复犯同样错误**的概率超过 70% |
| 综合结论 | 瓶颈不在模型智能，而在**运行环境设计** |

### 三个范式转变

| 旧范式 | 新范式 | 转变驱动力 |
|:--|:--|:--|
| 一次性提示（one-shot prompt） | 持续多会话协作 | 任务复杂度超出单窗口容量 |
| 优化模型本身 | 优化模型的运行环境 | 环境是瓶颈，不是模型 |
| 单 Agent 完成全部工作 | 多 Agent 分工协作 | 专用 Agent 比通用 Agent 更可靠 |

### 三种典型失败模式

| 失败模式 | 表现 | 根本原因 |
|:--|:--|:--|
| **上下文崩溃** | Agent 开始"忘事"或给出前后矛盾的答案 | 信息量超过上下文窗口，重要内容被截断 |
| **范围蔓延** | 一个小修改演变成无限延伸的重构 | 没有结构化执行流程，缺乏边界约束 |
| **记忆断层** | 新会话无法接上上次的工作，重头开始 | 进度没有持久化到文件系统 |
...
**必填字段：** 任务背景、子任务列表 (`[ ]`/`[x]`)、当前状态、阻塞项、关键决策记录、文件变更清单、验证记录
...
| 文件 | 操作 | 说明 |
|------|------|------|
| path/to/file.c | 修改 | 修复 UART FIFO 溢出 |
...
### 7.4 反压（Backpressure）机制

当 Agent 感受到任务超载时，**主动反馈**而不是默默失败：

| 超载信号 | 标准反应 | 错误反应 |
|:--|:--|:--|
| 子任务超过 5 步 | 暂停，请求人工拆分任务 | 继续执行，越做越乱 |
| 连续 3 次验证失败 | 升级为阻塞项，等待介入 | 一直重试，消耗上下文 |
| 需要信息在 Tier 3 | 显式声明"需要加载 X 文档" | 凭记忆编造，引入错误 |
| 操作影响超出预期 | 停下来，描述影响范围请确认 | 直接执行，造成不可逆损坏 |
...
└── .agents/                 ← 专用 Agent 工作区
    ...
    ├── bug-hunter/
    ├── docs-writer/
    └── {domain}-agent/      ← 领域专用 Agent（嵌入式、财经等）
```

### 9.2 会话启动脚本

**文件：** `harness/session-init.sh`

```bash
#!/bin/bash
# Harness Engineering — 会话启动脚本
# 用法: bash harness/session-init.sh [项目路径]
...
if [ -f "$PROJECT_DIR/PROGRESS.md" ]; then
  STATUS=$(grep -m1 "^\*\*状态:" "$PROJECT_DIR/PROGRESS.md" | sed 's/\*\*状态:\*\* //')
  echo "  📋 PROGRESS.md 存在 — 状态: ${STATUS:-未知}"
else
  echo "  ℹ️  无进行中任务"
fi
...
echo ""
echo "【Git 状态】"
cd "$PROJECT_DIR" && git log --oneline -3 2>/dev/null | sed 's/^/  /' || echo "  ⚠️  非 Git 仓库"
...
echo ""
echo "=== 检查完成: $PASS 通过 / $FAIL 缺失 ==="
[ "$FAIL" -gt 0 ] && echo "⚠️  请先创建缺失的 Tier 1 文件再开始工作" && exit 1
echo "✅ 环境就绪，可以开始工作"
```
...
```c
// 编译期错误标注宏 — GCC/Clang 可用（IAR/Keil 用 #pragma message 替代）
#define HARNESS_ERROR(category, desc, fix) \
    _Pragma("message \"ERROR: " category " - " desc "\"") \
    _Pragma("message \"  修复: " fix "\"")

// 用法示例
HARNESS_ERROR("中断安全", "isr_flag 未声明 volatile", "改为 volatile uint8_t isr_flag")
```
...
| 版本 | 日期 | 变更 |
|:--|:--|:--|
| v1.1 | 2026-05-15 | 补充失败模式分析、范式转变、数据支撑、反压机制、完整 PROGRESS 模板和 session-init.sh 脚本 |
